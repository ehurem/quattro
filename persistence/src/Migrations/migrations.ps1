...
$addMigrationCommand = "dotnet ef migrations add $currentMigration --context $dbContext --project $migrationProject --output-dir $migrationOutputDirectory"
$updateDbCommand = "dotnet ef database update $currentMigration --project $migrationProject --context $dbContext"

If($previousMigration -eq "" -and $previousMigration -eq [String]::Empty) {
    $createScriptCommand = "dotnet ef migrations script --context $dbContext --project $migrationProject -o '$migrationPath\$currentMigration.sql' --idempotent"
} Else {
    $createScriptCommand = "dotnet ef migrations script $previousMigration $currentMigration --context $dbContext --project $migrationProject -o '$migrationPath\$currentMigration.sql' --idempotent"
}
